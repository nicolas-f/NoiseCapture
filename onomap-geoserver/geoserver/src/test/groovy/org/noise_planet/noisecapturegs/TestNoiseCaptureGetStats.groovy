...
class TestNoiseCaptureGetStats extends JdbcTestCase {
    ...
    void setUp() {
        ...
        // Init schema
        st.execute(new File(TestNoiseCaptureGetStats.class.getResource("init_h2gis_start.sql").getFile()).text)
        st.execute(new File(TestNoiseCaptureGetStats.class.getResource("init_db.sql").getFile()).text)
        st.execute(new File(TestNoiseCaptureGetStats.class.getResource("init_h2gis_end.sql").getFile()).text)
        // Load timezone file
